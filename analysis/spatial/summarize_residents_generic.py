...
if len(sys.argv) != 2:
    print("Usage: python3 summarize_residents_generic.py scenario")
    sys.exit()

scenario = sys.argv[1]

input_file = f"{scenario}_activity_cleaned.csv"
resident_file = f"{scenario}_residents.txt"

residents = set()
with open(resident_file) as f:
    for line in f:
        residents.add(line.strip())
